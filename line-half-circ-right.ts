...
class LHCRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LHCRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLHCRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LHCRNode {
        var curr : LHCRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineHalfCircRight {

    curr : LHCRNode = new LHCRNode(0)
    dir : number = 1
