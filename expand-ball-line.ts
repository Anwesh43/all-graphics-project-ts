...
class EBLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new EBLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawEBLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : EBLNode {
        var curr : EBLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class ExpandBallLine {

    curr : EBLNode = new EBLNode(0)
    dir : number = 1
