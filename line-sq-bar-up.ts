...
class DrawingUtil {
    ...
    static drawLineSqBarUp(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(4), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, (size - barSize) * j, 0, () => {
                    context.fillRect(0, -barSize * dsc(1 + j), barSize, barSize * dsc(1 + j))
                })
            }
        })
    }

    static drawLSBUNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineSqBarUp(context, scale)
    }
}
