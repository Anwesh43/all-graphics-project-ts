...
class DrawingUtil {
    ...
    static drawTriRotRightLine(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w * dsc(3), h / 2, () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, size * 0.5 * j, -size * 0.5 * j, () => {
                    DrawingUtil.drawLine(context, 0, 0, size * 0.5 * dsc(j), -size * 0.5 * dsc(j) * (1 - 2 * j))
                })
            }
        })
    }

    static drawTRRLNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawTriRotRightLine(context, scale)
    }
}
...
class TriRotRightLine {

    curr: TRRLNode = new TRRLNode(0)
    dir: number = 1
