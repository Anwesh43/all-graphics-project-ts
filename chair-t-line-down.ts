...
class DrawingUtil {
    ...
    static drawChairTLineDown(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.restore()
            context.save()
            context.translate((size - barSize) * j, 0)
            context.fillRect(0, 0, barSize, barSize * dsc(2))
            context.restore()
        }
        context.restore()
    }
...
class CTLDNode {
    ...
    getNext(dir : number, cb : () => void) : CTLDNode {
