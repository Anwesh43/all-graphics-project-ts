...
class DrawingUtil {
    ...
    static drawMRLUNode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class MRLUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new MRLUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawMRLUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : MRLUNode {
        var curr : MRLUNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class MirrorRotLineUp {

    curr : MRLUNode = new MRLUNode(0)
    dir : number = 1
