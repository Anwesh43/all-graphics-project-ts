...
class DrawingUtil {
    ...
    static drawThreeSquareRight(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 2 + 1.5 * size) * dsc(3), h / 2, () => {
            ...
            for (let i = 0; i < 3; i++) {
                DrawingUtil.drawXY(context, (-w / 2 - size / 2) * (1 - dsc(0)), (h / 2 - size / 2) * (1 - dsc(1)) * (i - 1), () => {
...
class TSRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TSRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawTSRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : TSRNode {
        var curr : TSRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class ThreeSquareRight {

    curr : TSRNode = new TSRNode(0)
    dir : number = 1
