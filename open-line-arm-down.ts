...
class OLADNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new OLADNode(this.i + 1)
            this.next.prev = this 
        }
    }   

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawOLADNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : OLADNode {
        var curr : OLADNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class OpenLineArmDown {

    curr : OLADNode = new OLADNode(0)
    dir : number = 1
