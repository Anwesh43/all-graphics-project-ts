...
class MLRTNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new MLRTNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawMLRTNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): MLRTNode {
        var curr: MLRTNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class MultiLineRotTurn {

    curr: MLRTNode = new MLRTNode(0)
    dir: number = 1
