        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 - (h / 2) * dsc(5), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, -size * 0.5 * j, 0, () => {
                    ...
                    DrawingUtil.drawLine(context, 0, 0, 0, -lSize * ScaleUtil.divideScale(dsc(2), j, 2))
                })
            }
        })
    }

    static drawLABUNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineAltBentUp(context, scale)
    }
}
...
class LABUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LABUNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLABUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LABUNode {
        var curr: LABUNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
