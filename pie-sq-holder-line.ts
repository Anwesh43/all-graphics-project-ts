...
class PSHLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PSHLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPSHLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : PSHLNode {
        var curr : PSHLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class PieSqHolderLine {

    curr : PSHLNode = new PSHLNode(0)
    dir : number = 1
