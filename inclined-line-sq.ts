...
class DrawingUtil {
    ...
    static drawInclinedLineSq(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2 + size) * dsc(5), h / 2, () => {
            ...
            DrawingUtil.drawLine(context, 0, 0, -sqSize * dsc(3), 0)
        })
    }
...
class ILSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ILSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawILSNode(context, this.i, this.state.scale)
    }  
    ...
    getNext(dir : number, cb : () => void) : ILSNode {
        var curr : ILSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class InclinedLineSq {

    curr : ILSNode = new ILSNode(0)
    dir : number = 1
