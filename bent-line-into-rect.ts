...
class BLIRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BLIRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawBLIRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): BLIRNode {
        var curr: BLIRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class BentLineIntoRect {

    curr: BLIRNode = new BLIRNode(0)
    dir: number = 1
