...
class DrawingUtil {
    ...
    static drawPlusDivideBarUp(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawPDBUNode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class PDBUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PDBUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPDBUNode(context, this.i, this.state.scale)
    }
...
class PlusDivideBarUp {

    curr : PDBUNode = new PDBUNode(0)
    dir : number = 1
