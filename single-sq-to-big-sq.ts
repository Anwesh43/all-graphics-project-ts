        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2 + size) * dsc(4), () => {
            ...
            for (let j = 0; j < 4; j++) {
                DrawingUtil.drawXY(context, 0, 0, () => {
                    const factor : number = j == 0 ? 0 : Math.floor(dsc(j - 1)) * (j - 1)
                    const delta : number = j == 0 ? 0 : 1 
                    context.rotate(rot * factor  + rot * dsc(j) * delta)
...
class SSTBSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SSTBSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSSTBSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SSTBSNode {
        var curr : SSTBSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SingleSqToBigSq {

    curr : SSTBSNode = new SSTBSNode(0)
    dir : number = 1
