...
class SCRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SCRDNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawSCRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): SCRDNode {
        var curr: SCRDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class SideContainerRotDown {

    curr: SCRDNode = new SCRDNode(0)
    dir: number = 1
