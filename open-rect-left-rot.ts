...
class DrawingUtil {
    ...
    static drawOpenRectLeftRot(context : CanvasRenderingContext2D, scale : number) {
        const size : number = Math.min(w, h) / sizeFactor
        const dsc : (a : number) => number = (i : number) : number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(3), h / 2, () => {
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, (w / 2) - (w / 2 - size) * dsc(0), 0, () => {
                    context.rotate(-rot * dsc(1) * j)
                    DrawingUtil.drawLine(context, 0, 0, -size, 0)
                })
            }
            DrawingUtil.drawXY(context, -w / 2 + (w / 2 + size) * dsc(2), size, () => {
                DrawingUtil.drawLine(context, 0, 0, -size, 0)
            })
        })
    }

    static drawORLRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor  
        context.strokeStyle = colors[i]
        DrawingUtil.drawOpenRectLeftRot(context, scale)
    }
}
...
class OpenRectLeftRot {

    curr : ORLRNode = new ORLRNode(0)
    dir : number = 1
