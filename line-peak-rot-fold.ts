...
class DrawingUtil {
    ...
    static drawLine(context: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number) {
        if (Math.abs(x1 - x2) < 0.1 && Math.abs(x2 - y2) < 0.1) {
            return
        }
        context.beginPath()
    ...
    static drawLinePeakRotFold(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawLPRFNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLinePeakRotFold(context, scale)
    }
}
...
class LPRFNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPRFNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLPRFNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LPRFNode {
        var curr: LPRFNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LinePeakRotFold {

    curr: LPRFNode = new LPRFNode(0)
    dir: number = 1
