...
class DrawingUtil {
    ...
    static drawLineRotSqHold(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawLRSHNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineRotSqHold(context, scale)
    }
}
...
class LRSHNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRSHNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLRSHNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LRSHNode {
        let curr: LRSHNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
