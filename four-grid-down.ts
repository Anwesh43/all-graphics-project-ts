...
class FGDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new FGDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawFGDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : FGDNode {
        var curr : FGDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
