...
class BSARRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BSARRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSARRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BSARRNode {
        var curr : BSARRNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class BiSqAltRotRight {

    curr : BSARRNode = new BSARRNode(0)
    dir : number = 1
