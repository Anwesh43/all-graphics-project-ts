...
class DrawingUtil {
    ...
    static drawSBLRRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class SBLRRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SBLRRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSBLRRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SBLRRNode {
        var curr : SBLRRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
