...
class DrawingUtil {
    ...
    static drawSideWiseMidLine(context : CanvasRenderingContext2D, scale : number) {
        ...
        //console.log("SC1, SC2, SC3,", sc1, sc2, sc3, sc4)
        context.save()
...
class SWMLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SWMLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSWMLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SWMLNode {
        var curr : SWMLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SideWiseMidLine {

    curr : SWMLNode = new SWMLNode(0)
    dir : number = 1
    draw(context : CanvasRenderingContext2D) {
