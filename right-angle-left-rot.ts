        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2) * dsc(5), h / 2, () => {
            ...
            for (let j = 0; j < 2; j++) {
                ...
                DrawingUtil.drawXY(context, 0, size, () => {
                    ...
                    DrawingUtil.drawLine(context, 0, -size, 0, -size + size * dsc(3))
                })
            }
        })
    }
...
class RALRNode {
    ...
    constructor(private i: number) {
        if (this.i < colors.length - 1) {
            this.next = new RALRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawRALRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): RALRNode {
        var curr: RALRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class RightAngleLeftRot {

    curr: RALRNode = new RALRNode(0)
    dir: number = 1
