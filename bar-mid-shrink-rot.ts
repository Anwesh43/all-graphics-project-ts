...
class BMSRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BMSRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBMSRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BMSRNode {
        var curr : BMSRNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarMidShrinkRot {

    curr : BMSRNode = new BMSRNode(0)
    dir : number = 1
