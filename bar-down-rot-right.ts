...
class BDRRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BDRRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawBDRRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): BDRRNode {
        var curr: BDRRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class BarDownRotRight {

    curr: BDRRNode = new BDRRNode(0)
    dir: number = 1
