...
class DLTRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new DLTRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawDLTRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : DLTRNode {
        var curr : DLTRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class DivideLineTRot {

    curr : DLTRNode = new DLTRNode(0)   
    dir : number = 1
