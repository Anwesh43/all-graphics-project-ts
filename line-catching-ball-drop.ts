...
class LCBDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LCBDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLCBDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LCBDNode {
        var curr : LCBDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineCatchingBallDrop {

    curr : LCBDNode = new LCBDNode(0)
    dir : number = 1
