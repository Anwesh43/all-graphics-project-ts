...
class DrawingUtil {
    ...
    static drawFollowRectLine(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 2 + size) * dsc(3), h / 2, () => {
            ...
            context.fillRect(0, -size / 4, size * dsc(1), size / 2)
        })
    }
    ...
    static drawFRLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawFollowRectLine(context, scale)
    }
}
...
class FRLNode {
    ...
    getNext(dir : number, cb : () => void) : FRLNode {
        var curr : FRLNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
