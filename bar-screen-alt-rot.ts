...
class BSARNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BSARNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSARNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BSARNode {
        var curr : BSARNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarScreenAltRot {

    curr : BSARNode = new BSARNode(0)
    dir : number = 1
