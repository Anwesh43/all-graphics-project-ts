...
class SDRLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SDRLNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawSDRLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): SDRLNode {
        var curr: SDRLNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
