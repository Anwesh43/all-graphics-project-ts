...
class SELLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SELLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSELLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SELLNode {
        var curr : SELLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SqExtendLineLeft {

    curr : SELLNode = new SELLNode(0)
    dir : number = 1
