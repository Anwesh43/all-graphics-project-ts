...
class DrawingUtil {
    ...
    static drawBiLineDropDown(context : CanvasRenderingContext2D, scale : number) {
...
class BLDDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BLDDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBLDDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BLDDNode {
        var curr : BLDDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BiLineDropDown {

    curr : BLDDNode = new BLDDNode(0)
    dir : number = 1
