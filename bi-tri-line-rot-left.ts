...
class DrawingUtil {
    ...
    static drawBTLRLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawBiTriLineRotLeft(context, scale)
    }
}
...
class BTLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BTLRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBTLRLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BTLRNode {
        var curr : BTLRNode = this.prev
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BiTriLineRotLeft {

    curr : BTLRNode = new BTLRNode(0)
    dir : number = 1
