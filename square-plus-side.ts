...
class SPSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SPSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSPSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SPSNode {
        var curr : SPSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquarePlusSide {

    curr : SPSNode = new SPSNode(0)
    dir : number = 1
