...
class DrawingUtil {
    ...
    static drawLinePathSqHolder(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(6), h / 2, () => {
            DrawingUtil.drawXY(context, size * dsc(3), -size + (size - sqSize) * dsc(5), () => {
...
class LPSHNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPSHNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLPSHNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LPSHNode {
        var curr: LPSHNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LinePathSqHolder {

    curr: LPSHNode = new LPSHNode(0)
    dir: number = 1
