...
class DrawingUtil {
    ...
    static drawSquareUpBiDivide(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.save()
            context.translate((-size  / 2) * (1 - dsc(2)) * j, 0)
...
class SUBDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SUBDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSUBDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SUBDNode {
        var curr : SUBDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquareUpBiDivide {

    curr : SUBDNode = new SUBDNode(0)
    dir : number = 1
