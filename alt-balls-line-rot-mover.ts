...
class ABLRMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ABLRMNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawABLRMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : ABLRMNode {
        var curr : ABLRMNode = this.next 
        if (dir == -1) {
            curr = this.prev 
        }
        if (curr) {
...
class AltBallsLineRotMover {

    curr : ABLRMNode = new ABLRMNode(0)
    dir : number = 1
