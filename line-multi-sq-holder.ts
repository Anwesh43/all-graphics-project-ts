...
class DrawingUtil {
    ...
    static drawLineMultiSqHolder(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.translate(size * 0.5 * sc2, 0)
        ...
        context.fillRect(-size * 0.5 * sc3, -size / 2, size * sc3, size)
        context.restore()
    }

    static drawLMSHNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineMultiSqHolder(context, scale)
    }
}
...
class LMSHNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LMSHNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLMSHNode(context, this.i, this.state.scale)
    }
