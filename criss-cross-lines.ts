...
class DrawingUtil {
    ...
    static drawCrissCrossLine(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.save()
            context.rotate(deg * j * Math.floor(sc1))
            DrawingUtil.drawLine(context, size * 0.5 * sc2, size * 0.5 * sc1, -size * 0.5 * sc2, -size * 0.5 * sc1)
            context.restore()
            context.restore()
        }
...
class CCLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new CCLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawCCLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : CCLNode {
        var curr : CCLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class CrissCrossLines {

    curr : CCLNode = new CCLNode(0)
    dir : number = 1
