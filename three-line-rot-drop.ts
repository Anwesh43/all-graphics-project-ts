...
const scGap: number = (0.01 * (lines + 1)) / parts
...
class DrawingUtil {
    ...
    static drawThreeLineRotDrop(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        const gap: number = (w / 2) / lines
        DrawingUtil.drawXY(context, w / 2, h / 2, () => {
            let x: number = 0
            for (let j = 0; j < lines; j++) {
                const currX: number = j === 0 ? 0 : x + gap * dsc(j * 2)
                DrawingUtil.drawXY(context, currX, -h * 0.5 * (1 - dsc(0)) + h * 0.5 * dsc(parts - 1), () => {
...
class ThreeLineRotDrop {

    curr: TLRDNode = new TLRDNode(0)
    dir: number = 1
