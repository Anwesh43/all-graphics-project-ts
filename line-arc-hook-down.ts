...
class DrawingUtil {
    ...
    static drawLineArcHookDown(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawLAHDNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.strokeStyle = colors[i]
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        DrawingUtil.drawLineArcHookDown(context, scale)
    }
}
...
class LAHDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LAHDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLAHDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LAHDNode {
        var curr : LAHDNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        } 
        if (curr) {
...
class LineArcHookDown {

    curr : LAHDNode = new LAHDNode(0)
    dir : number = 1
