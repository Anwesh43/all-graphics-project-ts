...
class LRTPNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRTPNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLRTPNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LRTPNode {
        var curr : LRTPNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
