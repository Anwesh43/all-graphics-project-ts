...
class BSLDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BSLDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSLDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BSLDNode {
        var curr : BSLDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BigShortLineDown {

    curr : BSLDNode = new BSLDNode(0)
    dir : number = 1
