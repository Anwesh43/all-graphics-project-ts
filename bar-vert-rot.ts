...
class BVRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BVRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBVRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BVRNode {
        var curr : BVRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
