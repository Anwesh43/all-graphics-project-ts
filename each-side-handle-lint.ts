...
class DrawingUtil {
    ...
    static drawEachSideHandleLine(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawESHLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawEachSideHandleLine(context, scale)
    }
 }
...
class ESHLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ESHLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawESHLNode(context, this.i, this.state.scale)
    }
...
class EachSideHandleLine {

    curr : ESHLNode = new ESHLNode(0)
    dir : number = 1
