...
class SSCNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SSCNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSSCNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SSCNode {
        var curr : SSCNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class StrokeSquareCross {

    curr : SSCNode = new SSCNode(0)
    dir : number = 1
