...
class DrawingUtil {
    ...
    static drawRotBiArmLine(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.fillRect(0, -size / 2, size * 0.4, size * sc4)
...
class RBALNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RBALNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRBALNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : RBALNode {
        var curr : RBALNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
