...
class DrawingUtil {
    ...
    static drawLineAltSideDown(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawLASDNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawLineAltSideDown(context, scale)
    }
}
...
class LASDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LASDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLASDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LASDNode {
        var curr : LASDNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineAltSideDown {

    curr : LASDNode = new LASDNode(0)
    dir : number = 1
