...
class DrawingUtil {
    ...
    static drawOpenShiftLineDown(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(4), () => {
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, size * dsc(1), 0, () => {
                    context.rotate(rot * dsc(3) + deg * dsc(2) * (1 - 2 * j))
    ...
    static drawOSLDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawOpenShiftLineDown(context, scale)
    }
}
...
class OpenShiftLineDown {

    curr: OSLDNode = new OSLDNode(0)
    dir: number = 1
