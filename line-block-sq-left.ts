...
class DrawingUtil {
    ...
    static drawLBSLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineBlockSqLeft(context, scale)
    }
}
...
class LBSLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LBSLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLBSLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LBSLNode {
        var curr : LBSLNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineBlockSqLeft {

    curr : LBSLNode = new LBSLNode(0)
    dir : number = 1
