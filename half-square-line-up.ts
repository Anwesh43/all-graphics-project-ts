...
class HSLUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new HSLUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawHSLUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : HSLUNode {
        var curr : HSLUNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class HalfSquareLineUp {

    curr : HSLUNode = new HSLUNode(0)
    dir : number = 1
