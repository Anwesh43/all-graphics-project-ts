...
class DrawingUtil {
    ...
    static drawBarPlusRotUp(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawBPRUNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawBarPlusRotUp(context, scale)
    }
}
...
class BPRUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BPRUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBPRUNode(context, this.i, this.state.scale)
    }
