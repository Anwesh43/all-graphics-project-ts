...
class SCCNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SCCNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSCCNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SCCNode {
        var curr : SCCNode = this.prev 
        if (dir == 1) {
            curr = this.next
        }
        if (curr) {
...
class SquareCornerCircle {
    curr : SCCNode = new SCCNode(0)
    dir : number = 1
