...
class SLDHNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SLDHNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSLDHNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SLDHNode {
        var curr : SLDHNode = this.prev
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquareLineDivideHolder {

    curr : SLDHNode = new SLDHNode(0)
    dir : number = 1
