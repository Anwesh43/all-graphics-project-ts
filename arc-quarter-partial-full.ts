...
class AQPFNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new AQPFNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawAQPNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): AQPFNode {
        var curr: AQPFNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class ArcQuarterPartialFull {

    curr: AQPFNode = new AQPFNode(0)
    dir: number = 1
