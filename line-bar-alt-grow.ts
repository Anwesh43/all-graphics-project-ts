...
class LBAGNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LBAGNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLBAGNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LBAGNode {
        var curr : LBAGNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineBarAltGrow {

    curr : LBAGNode = new LBAGNode(0)
    dir : number = 1
