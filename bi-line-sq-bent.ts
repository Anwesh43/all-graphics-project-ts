...
class BLSBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BLSBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBLSBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BLSBNode {
        var curr : BLSBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
