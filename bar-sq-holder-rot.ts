...
class BSHRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length) {
            this.next = new BSHRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSHRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BSHRNode {
        var curr : BSHRNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarSqHolderRot {

    curr : BSHRNode = new BSHRNode(0)
    dir : number = 1
