...
class DrawingUtil {
    ...
    static drawRightLineIntoBox(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.restore()
        context.fillRect(0, -size, size * sc1, size)
        context.restore()
    }
...
class RLIBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RLIBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRLIBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : RLIBNode {
        var curr : RLIBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class RightlineIntoBox {

    curr : RLIBNode = new RLIBNode(0)
    dir : number = 1
