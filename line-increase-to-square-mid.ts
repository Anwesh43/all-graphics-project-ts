...
class DrawingUtil {
    ...
    static drawLineIncreaseToSquareMid(context : CanvasRenderingContext2D, scale : number) {
...
class LITSMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LITSMNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLITSMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LITSMNode {
        var curr : LITSMNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineIncreaseToSquareMid {

    curr : LITSMNode = new LITSMNode(0)
    dir : number = 1
