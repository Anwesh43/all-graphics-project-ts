...
class HBRCNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new HBRCNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawHBRCNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): HBRCNode {
        var curr: HBRCNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class HalfBoxRotComplete {

    curr: HBRCNode = new HBRCNode(0)
    dir: number = 1
