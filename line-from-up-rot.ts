...
class DrawingUtil {
    ...
    static drawLFURNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineFromUpRot(context, scale)
    }
}
...
class LFURNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LFURNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLFURNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LFURNode {
        var curr : LFURNode = this.prev 
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
