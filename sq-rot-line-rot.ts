...
class DrawingUtil {
    ...
    static drawSRLRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        context.lineWidth = Math.min(w, h) / strokeFactor 
        DrawingUtil.drawSqRotLineRot(context, scale)
    }
}
...
class SRLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SRLRNode(this.i + 1)
            this.next.prev = this 
        }
    }
    ...
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSRLRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SRLRNode {
        var curr : SRLRNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
