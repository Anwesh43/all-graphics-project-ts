...
class RLBSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RLBSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRLBSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : RLBSNode {
        var curr : RLBSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class RotLineBarSide {

    curr : RLBSNode = new RLBSNode(0)
    dir : number = 1
