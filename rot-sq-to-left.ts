...
class RSTLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RSTLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRSTLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : RSTLNode {
        var curr : RSTLNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
