...
class BPLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BPLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBPLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BPLNode {
        var curr : BPLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
