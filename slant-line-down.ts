...
class DrawingUtil {
    ...
    static drawSlantLineDown(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawSLDNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawSlantLineDown(context, scale)
    }
}
...
class SLDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SLDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSLDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SLDNode {
        var curr : SLDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
