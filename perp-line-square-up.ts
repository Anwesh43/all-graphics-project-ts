...
class DrawingUtil {
    ...
    static drawPerpLineSquareUp(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            context.save()
            context.translate(0, size * (1 - 2 * j) * dsc(2))
...
class PLSUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PLSUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPLSUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : PLSUNode {
        var curr : PLSUNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class PerpLineSquareUp {

    curr : PLSUNode = new PLSUNode(0)
    dir : number = 1
