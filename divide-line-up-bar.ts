...
class DrawingUtil {
    ...
    static drawDivideLineUpBar(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawDLUBNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawDivideLineUpBar(context, scale)
    }
}
...
class DLUBNode {
    ...
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawDLUBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : DLUBNode {
        let curr : DLUBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class DivideLineUpBar {

    curr : DLUBNode = new DLUBNode(0)
    dir : number = 1
