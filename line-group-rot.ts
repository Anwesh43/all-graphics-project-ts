...
class DrawingUtil {
    ...
    static drawLineGroupRot(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2) * dsc(5), h / 2, () => {
            ...
            DrawingUtil.drawXY(context, size, -h / 2 + (h / 2 + size) * dsc(3), () => {
                DrawingUtil.drawLine(context, 0, 0, 0, -size)
            })
        })
    }
    
    static drawLGRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.strokeStyle = colors[i]
        context.lineWidth = Math.min(w, h) / strokeFactor 
        DrawingUtil.drawLineGroupRot(context, scale)
    }
}
