...
class DrawingUtil {
    ...
    static drawDropLineCatchSq(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(4), h / 2, () => {
            DrawingUtil.drawXY(context, -w * 0.25 * (1 - dsc(2)), -h * 0.5 * (1 - dsc(0)), () => {
    ...
    static drawDLCSNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawDropLineCatchSq(context, scale)
    }
}
...
class DropLineCatchSq {

    curr: DLCSNode = new DLCSNode(0)
    dir: number = 1
