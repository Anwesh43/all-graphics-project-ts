...
const circleSizeFactor : number = 11.2 
...
class CircleBarExtender {

    curr : CBENode = new CBENode(0)
    dir : number = 1
...
class Renderer {
    ...
    cbe : CircleBarExtender = new CircleBarExtender()
    animator : Animator = new Animator()

    render(context : CanvasRenderingContext2D) {
        this.cbe.draw(context)
    }

    handleTap(cb : () => void) {
        this.cbe.startUpdating(() => {
            this.animator.start(() => {
                cb()
                this.cbe.update(() => {
                    cb()
