    ...
    static drawBarRotCircleDown(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2 + size) * dsc(3), () => {
            ...
            DrawingUtil.drawXY(context, w * 0.5 * (1 - dsc(0)), 0, () => {
                ...
                DrawingUtil.drawCircle(context, size / 3, 0, size / 3)
            })
        })
    }
...
class BRCDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BRCDNode(this.i + 1)
            this.next.prev = this 
        }
    }
    
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBRCDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BRCDNode {
        var curr : BRCDNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarRotCircleDown {

    curr : BRCDNode = new BRCDNode(0)
    dir : number = 1
