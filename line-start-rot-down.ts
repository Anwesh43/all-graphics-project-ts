...
class DrawingUtil {
    ...
    static drawLineStartRotDown(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w * 0.5 * dsc(0), h / 2 + (h / 2) * dsc(3), () => {
    ...
    static drawLSRDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        ...
        DrawingUtil.drawLineStartRotDown(context, scale)
    }
}
...
class LSRDNode {
    ...
    prev: LSRDNode | null = null
    next: LSRDNode | null = null
    ...
    getNext(dir: number, cb: () => void): LSRDNode {
        var curr: LSRDNode | null = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
