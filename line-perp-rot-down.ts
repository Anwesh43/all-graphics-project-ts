...
class DrawingUtil {
    ...
    static drawLinePerpRotDown(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2 + size) * dsc(3), () => {
            ...
            DrawingUtil.drawLine(context, -size * 0.5 * dsc(2), 0, size * 0.5 * dsc(2), 0)
            for (let j = 0; j < 2; j++) {
...
class LPRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPRDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLPRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LPRDNode {
        var curr : LPRDNode = this.prev
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LinePerpRotDown {

    curr : LPRDNode = new LPRDNode(0)
    dir : number = 1
