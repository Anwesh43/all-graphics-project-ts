...
class DrawingUtil {
    ...
    static drawLineArcSweepDown(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 4) * dsc(2), h / 2 + (h / 2) * dsc(4), () => {
            ...
            DrawingUtil.drawArc(context, 0, 0, size, -90, -90 + 90 * dsc(1))
        })
    }
...
class LASDNode {
    ...
    prev: LASDNode | null = null
    next: LASDNode | null = null
    ...
    getNext(dir: number, cb: () => void): LASDNode {
        var curr: LASDNode | null = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
