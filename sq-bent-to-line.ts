...
class DrawingUtil {
    ...
    static drawSBTLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawSqBentToLine(context, scale)
    }
}
...
class SBTLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SBTLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSBTLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SBTLNode {
        var curr : SBTLNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
