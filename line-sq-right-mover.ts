...
class DrawingUtil {
    ...
    static drawLineSqRightMove(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawLSRMNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineSqRightMove(context, scale)
    }
}
...
class LSRMNode {
    ...
    getNext(dir : number, cb : () => void) : LSRMNode {
        var curr : LSRMNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
