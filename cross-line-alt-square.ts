...
class CLASNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new CLASNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawCLASNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : CLASNode {
        var curr : CLASNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class CrossLineAltSquare {

    curr : CLASNode = new CLASNode(0)
    dir : number = 1
