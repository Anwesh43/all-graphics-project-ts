...
class LADSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LADSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLADSNode(context, this.i, this.state.scale)
    }
...
class LineArcDownShift {

    curr : LADSNode = new LADSNode(0)
    dir : number = 1
