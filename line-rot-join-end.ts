...
class DrawingUtil {
    ...
    static drawLineRotJoinEnd(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(5), () => {
            ...
            DrawingUtil.drawXY(context, size, -size, () => {
                DrawingUtil.drawLine(context, 0, 0, -size * dsc(3), size * dsc(3))
            })
        })
    }

    static drawLRJENode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineRotJoinEnd(context, scale)
    }
}
...
class LineRotJoinEnd {

    curr: LRJENode = new LRJENode(0)
    dir: number = 1
