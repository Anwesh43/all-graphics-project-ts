...
class BSRLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BSRLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BSRLNode {
        var curr : BSRLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarStackRotateLeft {

    curr : BSRLNode = new BSRLNode(0)
    dir : number = 1
