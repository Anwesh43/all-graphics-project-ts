...
class TLTSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TLTSNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawTLTSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : TLTSNode {
        var curr : TLTSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
