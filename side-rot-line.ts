...
class SRLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SRLNode(this.i + 1)
            this.next.prev = this 
        }
    }
    
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSRLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SRLNode {
        var curr : SRLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
