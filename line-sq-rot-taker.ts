...
class LSRTNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LSRTNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLSRTNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LSRTNode {
        var curr : LSRTNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineSqRotTaker {

    curr : LSRTNode = new LSRTNode(0)
    dir : number = 1
