...
class DrawingUtil {
    ...
    static drawLineRightRotMove(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2, () => {
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, w * 0.25 * (1 - 2 * j) * (1 - dsjk(j + 1, 0)) - w * 0.5 * (1 - j) * dsc(3), h * 0.5 * dsc(3) * j, () => {
...
class LRRMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRRMNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLRRMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LRRMNode {
        var curr: LRRMNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineRightRotMove {

    curr: LRRMNode = new LRRMNode(0)
    dir: number = 1
...
class Renderer {
    ...
    lrrm: LineRightRotMove = new LineRightRotMove()
    animator: Animator = new Animator()

    render(context: CanvasRenderingContext2D) {
        this.lrrm.draw(context)
    }

    handleTap(cb: () => void) {
        this.lrrm.startUpdating(() => {
            this.animator.start(() => {
                cb()
                this.lrrm.update(() => {
                    this.animator.stop()
