...
const finalRot: number = 0.75 * Math.PI
...
class TLRUNode {
    ...
    prev: TLRUNode | null = null
    next: TLRUNode | null = null
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TLRUNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawTLRUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): TLRUNode {
        var curr: TLRUNode | null = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class TrisectLineRotUp {

    curr: TLRUNode = new TLRUNode(0)
    dir: number = 1
