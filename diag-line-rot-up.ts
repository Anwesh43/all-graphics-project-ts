...
class DrawingUtil {
    ...
    static drawDiagLineRotUp(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(3), h / 2 - h * 0.5 * dsc(3), () => {
            DrawingUtil.drawXY(context, 0, -h * 0.5 * (1 - dsc(0)), () => {
                DrawingUtil.drawLine(context, 0, 0, 0, -size)
            })
            DrawingUtil.drawXY(context, w * 0.5 * (1 - dsc(1)), -h * 0.5 * (1 - dsc(1)), () => {
    ...
    static drawDLRUNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        ...
        DrawingUtil.drawDiagLineRotUp(context, scale)
    }
}
