...
class LSBDNode {
    ...
    prev: LSBDNode | null = null
    next: LSBDNode | null = null
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LSBDNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLSBDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LSBDNode {
        var curr: LSBDNode | null = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineSemiBentDown {

    curr: LSBDNode = new LSBDNode(0)
    dir: number = 1
