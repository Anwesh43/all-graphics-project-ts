...
class BELRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BELRNode(this.i + 1) 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBELRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BELRNode {
        var curr : BELRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarEdgeLineRot {

    curr : BELRNode = new BELRNode(0)
    dir : number = 1
