...
class DrawingUtil {
    ...
    static drawRSISNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawRotSideIntoSingle(context, scale)
    }
}
...
class RSISNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RSISNode(this.i + 1)
            this.next.prev = this 
        }
    }
    ...
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRSISNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : RSISNode {
        var curr : RSISNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class RotSideIntoSingle {

    curr : RSISNode = new RSISNode(0)
    dir : number = 1
