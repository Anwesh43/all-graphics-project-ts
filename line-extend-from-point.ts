...
class DrawingUtil {
    ...
    static drawLineExtendFromPoint(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(3), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, 0, 0, () => {
                    for (let k = 0; k < 2; k++) {
                        DrawingUtil.drawXY(context, 0, 0, () => {
                            context.rotate(rot *(1 + dsc(1))* k * (1 - 2 * j))
    ...
    static drawLEFPNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineExtendFromPoint(context, scale)
    }
}
...
class LineExtendFromPoint {

    curr : LEFPNode = new LEFPNode(0)
    dir : number = 1
