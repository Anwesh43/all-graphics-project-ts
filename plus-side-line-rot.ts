...
const lineSizeFactor : number = 14.9 
...
class DrawingUtil {
    ...
    static drawPlusSideLineRot(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawPSLRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawPlusSideLineRot(context, scale)
    }
}
...
class PSLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PSLRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPSLRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : PSLRNode {
        var curr : PSLRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class PlusSideLineRot {

    curr : PSLRNode = new PSLRNode(0)
    dir : number = 1
