...
class DrawingUtil {
    ...
    static drawLineParallelJoiner(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.translate(0, -size * 0.25)
            DrawingUtil.drawLine(context, -size * 0.5 * sc1, 0, size * 0.5 * sc1, 0)
            for (let k = 0; k < 2; k++) {
                context.save()
                context.translate(0, size * 0.25)
                context.scale(1 - 2 * k, 1)
                DrawingUtil.drawLine(context, 0, 0, size * 0.25 * sc2, size * 0.25 * sc2)
                context.restore()
            }
            context.restore()
        }
...
class LPJNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPJNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLPJNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LPJNode {
        var curr : LPJNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineParallelJoiner {

    curr : LPJNode = new LPJNode(0)
    dir : number = 1
