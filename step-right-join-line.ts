...
class DrawingUtil {
    ...
    static drawStepRightJoinLine(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(5), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, size * 0.5 * j, -size + size * 0.5 * j, () => {
                    for (let k = 0; k < 2; k++) {
...
class SRJLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SRJLNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawSRJLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): SRJLNode {
        var curr: SRJLNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class StepRightJoinLine {

    curr: SRJLNode = new SRJLNode(0)
    dir: number = 1
