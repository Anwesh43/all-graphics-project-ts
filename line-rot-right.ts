...
class LRRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLRRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LRRNode {
        var curr: LRRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
