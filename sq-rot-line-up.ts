...
class DrawingUtil {
    ...
    static drawSRLUNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawSqRotLineUp(context, scale)
    }
}
...
class SRLUNode {
    ...
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSRLUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SRLUNode {
        var curr : SRLUNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
