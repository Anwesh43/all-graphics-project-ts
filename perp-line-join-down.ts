...
class DrawingUtil {
    ...
    static drawPLJDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawPerpLineJoinDown(context, scale)
    }
}
...
class PLJDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PLJDNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawPLJDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): PLJDNode {
        var curr: PLJDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class PerpLineJoinDown {

    curr: PLJDNode = new PLJDNode(0)
    dir: number = 1
