...
class BCCNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BCCNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBCCNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BCCNode {
        var curr : BCCNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BlockCrossContainer {

    curr : BCCNode = new BCCNode(0)
    dir : number = 1
