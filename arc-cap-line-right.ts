...
class DrawingUtil {
    ...
    static drawArcCapLineRight(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(3), h / 2, () => {
            DrawingUtil.drawArc(context, size, -90, 90 * dsc(0))
    ...
    static drawACLRNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawArcCapLineRight(context, scale)
    }
}
...
class ACLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ACLRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawACLRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): ACLRNode {
        var curr: ACLRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class ArcCapLineRight {

    curr: ACLRNode = new ACLRNode(0)
    dir: number = 1
