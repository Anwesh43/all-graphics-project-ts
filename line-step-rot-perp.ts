...
class DrawingUtil {
    ...
    static drawLineStepRotPerp(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2) * dsc(4), h / 2, () => {
            ...
            DrawingUtil.drawXY(context, w * 0.25 * (1 - dsc(3)), -h * 0.5 * (1 - dsc(1)), () => {
    ...
    static drawLSRPNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineStepRotPerp(context, scale)
    }
}
...
class LineStepRotPrep {

    curr: LSRPNode = new LSRPNode(0)
    dir: number = 1
