...
class DrawingUtil {
        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2 + size) * dsc(4), h / 2, () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, 0, 0, () => {
                    ...
                    DrawingUtil.drawXY(context, 0, size * 0.5, () => {
    ...
    static drawFELLNode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class FELLNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new FELLNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawFELLNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : FELLNode {
        var curr : FELLNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class ForkExpandLineLeft {

    curr : FELLNode = new FELLNode(0)
    dir : number = 1
