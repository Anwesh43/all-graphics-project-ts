...
class RAHDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RAHDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRAHDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : RAHDNode {
        var curr : RAHDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class RightAngleHolderDown {

    curr : RAHDNode = new RAHDNode(0)
    dir : number = 1
