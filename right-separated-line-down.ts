...
class DrawingUtil {
    ...
    static drawRightSeparatedLineDown(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(3), () => {
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, (w / 4) * (1 - 2 * j) * (1 - dsc(1)), 0, () => {
                    context.rotate(rot * (1 + j) * (2 * j - 1) * (1 - dsc(2)))
    ...
    static drawRSLDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        ...
        DrawingUtil.drawRightSeparatedLineDown(context, scale)
    }
}
...
class RightSeparatedLineDown {

    curr: RSLDNode = new RSLDNode(0)
    dir: number = 1
