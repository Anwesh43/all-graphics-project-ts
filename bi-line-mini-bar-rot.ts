...
class BLMBRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BLMBRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBLMBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BLMBRNode {
        var curr : BLMBRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BiLineMiniBar {
    ...
    curr : BLMBRNode = new BLMBRNode(0)
    dir : number = 1
