...
class DrawingUtil {
    ...
    static drawExternalArcRightAngle(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.save()
        context.translate(w / 2, h / 2 + (h / 2 + size + r) * sc4)
    ...
    static drawEARANode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class EARANode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new EARANode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawEARANode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : EARANode {
        var curr : EARANode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class ExternalArcRightAngle {

    curr : EARANode = new EARANode(0)
    dir : number = 1
