...
class DrawingUtil {
    ...
    static drawULRRNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawUpLineRightRoot(context, scale)
    }
}
...
class ULRRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ULRRNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawULRRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): ULRRNode {
        var curr: ULRRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class UpLineRightRot {

    curr: ULRRNode = new ULRRNode(0)
    dir: number = 1
