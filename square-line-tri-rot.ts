...
class DrawingUtil {
    ...
    static drawSquareLineTriRot(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.fillRect(-size / 2 - context.lineWidth / 2, -size * dsc(0), size + context.lineWidth, size * dsc(0))
...
class SLTRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SLTRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSLTRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SLTRNode {
        var curr : SLTRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SquareLineTriRot {

    curr : SLTRNode = new SLTRNode(0)
    dir : number = 1
