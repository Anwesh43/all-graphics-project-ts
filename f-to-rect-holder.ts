...
class FTRHNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new FTRHNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawFTRHNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : FTRHNode {
        var curr : FTRHNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
