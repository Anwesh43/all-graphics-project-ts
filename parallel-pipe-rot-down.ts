...
class DrawingUtil {
    ...
    static drawParallelPipeRotDown(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawPPRDNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawParallelPipeRotDown(context, scale)
    }
}
...
class PPRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PPRDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPPRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : PPRDNode {
        var curr : PPRDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
