...
class ASRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ASRDNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawASRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): ASRDNode {
        var curr: ASRDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
