...
class DrawingUtil {
    ...
    static drawBentArcJoinRight(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawBAJRNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawBentArcJoinRight(context, scale)
    }
}
...
class BAJRNode {
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawBAJRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): BAJRNode {
        var curr: BAJRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class BentArcJoinRight {

    curr: BAJRNode = new BAJRNode(0)
    dir: number = 1
