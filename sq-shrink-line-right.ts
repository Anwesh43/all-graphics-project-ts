...
class State {
    ...
    update(cb : () => void) {
        if (Math.abs(this.prevScale - this.prevScale) > 1) {
...
class SSLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SSLRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSSLRNode(context, this.i, this.state.scale)
    }
...
class SqShrinkLineRight {

    curr : SSLRNode = new SSLRNode(0)
    dir : number = 1
