...
class DrawingUtil {
    ...
    static drawPartialLineRotJoiner(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawPLRJNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawPartialLineRotJoiner(context, scale)
    }
}
...
class PLRJNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PLRJNode(this.i + 1)
            this.next.prev = this
        }
    }
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawPLRJNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): PLRJNode {
        var curr: PLRJNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class PartialLineRotJoiner {

    curr: PLRJNode = new PLRJNode(0)
    dir: number = 1
