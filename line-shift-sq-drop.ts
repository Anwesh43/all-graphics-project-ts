...
class DrawingUtil {
    ...
    static drawLineShiftSqDrop(context : CanvasRenderingContext2D, scale : number) {
        const size : number = Math.min(w, h) / sizeFactor
        const dsc : (number) => number = (i : number) : number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2 - (w / 2) * dsc(4), h / 2, () => {
            context.rotate(rot * dsc(3))
            DrawingUtil.drawXY(context, size * dsc(1), -h * 0.5 * (1 - dsc(0)), () => {
...
class LSSDNode {
    ...
    getNext(dir : number, cb : () => void) : LSSDNode {
