...
class RLCSNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RLCSNode(this.i +1 )
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRLCSNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : RLCSNode {
        var curr : RLCSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class RightLineCreateShift {
    
    curr : RLCSNode = new RLCSNode(0)
    dir : number = 1
...
class Renderer {
    ...
    rlcs : RightLineCreateShift = new RightLineCreateShift()
    animator : Animator = new Animator()

    render(context : CanvasRenderingContext2D) {
        this.rlcs.draw(context)    
    }
