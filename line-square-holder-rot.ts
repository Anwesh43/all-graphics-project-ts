...
class DrawingUtil {
    ...
    static drawLineSquareHolderRot(context : CanvasRenderingContext2D, scale : number) {
    ...
    static drawLSHRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineSquareHolderRot(context, scale)
    }
}
...
class LSHRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LSHRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLSHRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LSHRNode {
        var curr : LSHRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineSquareHolder {

    curr : LSHRNode = new LSHRNode(0)
    dir : number = 1
