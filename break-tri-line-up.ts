...
class DrawingUtil {
    ...
    static drawBreakTriLine(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2, h / 2, () => {
            context.rotate(rot * dsc(2))
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, (w / 2) * (1 - 2 * j) * (1 - dsc(3)), size, () => {
                    DrawingUtil.drawLine(context, 0, 0, size * (1 - 2 * j) * dsc(j), size * dsc(j))
                })
            }
        })
    }
    ...
    static drawBTLNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        ...
        DrawingUtil.drawBreakTriLine(context, scale)
    }
}
...
class BreakTriLineUp {

    curr: BTLUNode = new BTLUNode(0)
    dir: number = 1
