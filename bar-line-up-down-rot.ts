...
class DrawingUtil {
    ...
    static drawBarLineUpDownRot(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            context.fillRect(0, -barSize * sc2, size / 2, barSize * sc2)
            context.restore()
        }
...
class BLUDRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BLUDRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBLUDRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BLUDRNode {
        var curr : BLUDRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarLineUpDownRot {

    curr : BLUDRNode = new BLUDRNode(0)
    dir : number = 1
