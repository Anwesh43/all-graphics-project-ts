...
class DrawingUtil {
    ...
    static drawLineBentExtendDown(context: CanvasRenderingContext2D, scale: number) {
            ...
            DrawingUtil.drawXY(context, size / 2, 0, () => {
                ...
                DrawingUtil.drawXY(context, size / 2, -size / 2, () => {
                    ...
                    DrawingUtil.drawXY(context, 0, size / 2, () => {
                        DrawingUtil.drawLine(context, 0, 0, -size * 0.5 * dsc(4), 0)
                    })
                })
            })
        })
...
class LBEDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LBEDNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLBEDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LBEDNode {
        var curr: LBEDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineBentExtendDown {

    curr: LBEDNode = new LBEDNode(0)
    dir: number = 1
...
class Renderer {
    ...
    lbed: LineBentExtendDown = new LineBentExtendDown()
