...
class DrawingUtil {
    ...
    static drawLineAltRot(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.save()
        context.translate(w / 2 - (w / 2 + context.lineWidth) * sc4, h / 2)
        context.save()
        for (let j = 0; j < 2; j++) {
            context.save()
    ...
    static drawLARNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawLineAltRot(context, scale)
    }
}
