...
class SBLRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SBLRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSBLRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SBLRNode {
        var curr : SBLRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class SqBreakLeftRot {

    curr : SBLRNode = new SBLRNode(0)
    dir : number = 1
