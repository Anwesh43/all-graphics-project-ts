...
class BCURNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BCURNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawBCURNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): BCURNode {
        var curr: BCURNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class BlockCornerUpRight {

    curr: BCURNode = new BCURNode(0)
    dir: number = 1
