...
class TRRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new TRRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawTRRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : TRRNode {
        var curr : TRRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class TridentRotRight {

    curr : TRRNode = new TRRNode(0)
    dir : number = 1
