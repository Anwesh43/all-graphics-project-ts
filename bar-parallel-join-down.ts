...
class BPJDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BPJDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBPJDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BPJDNode {
        var curr : BPJDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarParallelJoinDown {

    curr : BPJDNode = new BPJDNode(0)
    dir : number = 1
