...
class DrawingUtil {
    ...
    static drawLineBlockSquareMover(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            DrawingUtil.drawLine(context, 0, -context.lineWidth / 2, size * dsc(1), -context.lineWidth / 2)
            context.restore()
        }
...
class LBSMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LBSMNode(this.i + 1)
            this.next.prev = this 
        }
    }
    
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLBSMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LBSMNode {
        var curr : LBSMNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineBlockSquareMover {

    curr : LBSMNode = new LBSMNode(0)
    dir : number = 1
