...
class DrawingUtil {
    ...
    static drawTriLineRotDown(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2 + size) * dsc(3), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, 0, 0, () => {
                    context.rotate((1 - 2 * j) * (Math.PI / 6) * dsc(1))
    ...
    static drawTLRDNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawTriLineRotDown(context, scale)
    }
}
