...
class DrawingUtil {
    ...
    static drawVertLinePie(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(3), () => {
            DrawingUtil.drawLine(context, -size / 2, 0, -size / 2 + size * dsc(0), 0)
            for (let j = 0; j < 2; j++) {
    ...
    static drawVLPNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawVertLinePie(context, scale)
    }
}
