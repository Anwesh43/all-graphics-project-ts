...
class RRLUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RRLUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRRLUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : RRLUNode {
        var curr : RRLUNode = this.prev 
        if (dir === 1) {
            curr = this.next 
        }
        if (curr) {
...
class RightRotLineUp {

    curr : RRLUNode = new RRLUNode(0)
    dir : number = 1
