...
class DrawingUtil {
    ...
    static drawLRIUNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineRotInclineUp(context, scale)
    }
}
...
class LRIUNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRIUNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLRIUNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LRIUNode {
        var curr : LRIUNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineRotInclineUp {

    curr : LRIUNode = new LRIUNode(0)
    dir : number = 1
