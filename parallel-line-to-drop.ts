...
class DrawingUtil {
    ...
    static drawParallelLineToDrop(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2, h / 2 + (h / 2) * dsc(3), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, 0, -size * (1 - dsc(1)) * (1 - 2 * j), () => {
...
class PLTDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PLTDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPLTDNode(context, this.i, this.state.scale)
    }
...
class ParallelLineToDrop {

    curr : PLTDNode = new PLTDNode(0)
    dir : number = 1
