...
class DrawingUtil {
    ...
    static drawRCLRNode(context: CanvasRenderingContext2D, i: number, scale: number) {
...
class RLCRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RLCRNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    getNext(dir: number, cb: () => void): RLCRNode {
        var curr: RLCRNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class RightCapLineRot {

    curr: RLCRNode = new RLCRNode(0)
    dir: number = 1
