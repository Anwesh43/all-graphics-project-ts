...
class DrawingUtil {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(4), h / 2, () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, 0, 0, () => {
                    ...
                    DrawingUtil.drawXY(context, (size - sqSize) * (1 - dsc(2)), -h * 0.5 * (1 - dsc(1)), () => {
                        context.fillRect(0, -2 * sqSize, sqSize, 2 * sqSize)
                    })
    ...
    static drawDBSRRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        context.fillStyle = colors[i]
        DrawingUtil.drawDropBiSqRotRight(context, scale)
    }
}
...
class DropBiSqRotRight {

    curr : DBSRRNode = new DBSRRNode(0)
    dir : number = 1
