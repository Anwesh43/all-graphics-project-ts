...
class DrawingUtil {
    ...
    static drawLFTBNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineFromTopBent(context, scale)
    }
}
...
class LFTBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LFTBNode(this.i + 1)
            this.next.prev = this 
        }
    }
    ...
    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLFTBNode(context, this.i, this.state.scale)
    }

    update(cb : () => void) {
        this.state.upadte(cb)
    }
    ...
    getNext(dir : number, cb : () => void) : LFTBNode {
        var curr : LFTBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineFromTopBent {

    curr : LFTBNode = new LFTBNode(0)
    dir : number = 1
