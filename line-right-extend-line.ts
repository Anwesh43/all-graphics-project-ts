...
class LRELNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRELNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLRELNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LRELNode {
        let curr: LRELNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineRightExtendLine {

    curr: LRELNode = new LRELNode(0)
    dir: number = 1
