...
class DrawingUtil {
    ...
    static drawFRect(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.fillRect(0, -size, size * sc3, size * 0.5)
        context.restore()
    }
...
class FRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new FRDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawFRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : FRDNode {
        var curr : FRDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
