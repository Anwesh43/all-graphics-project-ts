...
class DrawingUtil {
        ...
        DrawingUtil.drawXY(context, w * 0.5 * dsc(2), h / 2 + (h / 2) * dsc(4), () => {
            ...
            for (let j = 0; j < 2; j++) {
                DrawingUtil.drawXY(context, size, 0, () => {
                    context.rotate(Math.PI * 0.25 * (1 - 2 * j))
    ...
    static drawSOLRNode(context: CanvasRenderingContext2D, i: number, scale: number) {
...
class SOLRNode {
    ...
    prev: SOLRNode | null = null
    next: SOLRNode | null = null
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SOLRNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawSOLRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): SOLRNode {
        var curr: SOLRNode | null = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class SquareOpenLineRot {

    curr: SOLRNode = new SOLRNode(0)
    dir: number = 1
