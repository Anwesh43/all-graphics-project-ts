...
class DrawingUtil {
    ...
    static drawLPTANode(context : CanvasRenderingContext2D, i : number, scale : number) {
        context.strokeStyle = colors[i]
        context.lineWidth = Math.min(w, h) / strokeFactor 
        context.lineCap = 'round'
        DrawingUtil.drawLinePerpToArrow(context, scale)
    }
}
...
class LPTANode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPTANode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLPTANode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : LPTANode {
        var curr : LPTANode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
