...
class DrawingUtil {
    ...
    static drawRightAngleSquareMove(context : CanvasRenderingContext2D, scale : number) {
        ...
        context.fillRect(size - size * sc4, -size, size * sc4, size)
        context.restore()
    }
...
class RASMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new RASMNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawRASMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : RASMNode {
        var curr : RASMNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class RightAngleSquareMove {
    
    curr : RASMNode = new RASMNode(0)
    dir : number = 1
