    ...
    static drawRotMoreLineRight(context : CanvasRenderingContext2D, scale : number) {
        ...
        DrawingUtil.drawXY(context, w / 2 + (w / 2) * dsc(3), h / 2, () => {
            ...
            DrawingUtil.drawXY(context, 0, 0, () => {
                ...
                DrawingUtil.drawLine(context, 0, 0, 0, -size * 0.5 * ScaleUtil.divideScale(dsc(0), 0, 2))
                DrawingUtil.drawXY(context, 0, -size / 2, () => {
                    context.rotate(rot * dsc(2))
                    DrawingUtil.drawLine(context, 0, 0, 0, -size * 0.5 * ScaleUtil.divideScale(dsc(0), 1, 2))
                })               
            })
        })
    }

    static drawRMLRNode(context : CanvasRenderingContext2D, i : number, scale : number) {
...
class RotMoreLineRight {

    curr : RMLRNode = new RMLRNode(0)
    dir : number = 1
