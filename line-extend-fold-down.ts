...
class DrawingUtil {
    ...
    static drawLineExtendFoldDown(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawLEFDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineExtendFoldDown(context, scale)
    }
}
...
class LEFDNode {
    ...
    getNext(dir: number, cb: () => void): LEFDNode {
        var curr: LEFDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineExtendFoldDown {

    curr: LEFDNode = new LEFDNode(0)
    dir: number = 1
