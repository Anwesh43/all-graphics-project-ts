...
class DrawingUtil {
    ...
    static drawLPJDNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLinePartialJoinDown(context, scale)
    }
}
...
class LPJDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LPJDNode(this.i + 1)
            this.next.prev = this
        }
    }
    ...
    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLPJDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LPJDNode {
        var curr: LPJDNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LinePartialJoinDown {

    curr: LPJDNode = new LPJDNode(0)
    dir: number = 1
