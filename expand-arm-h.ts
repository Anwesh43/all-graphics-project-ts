...
class DrawingUtil {
    ...
    static drawExpandArmH(context : CanvasRenderingContext2D, scale : number) {
        const sc1 : number = ScaleUtil.divideScale(scale, 0, parts)
        const sc2 : number = ScaleUtil.divideScale(scale, 1, parts)
        const sc3 : number = ScaleUtil.divideScale(scale, 2, parts)
        const sc4 : number = ScaleUtil.divideScale(scale, 3, parts)
        const size : number = Math.min(w, h) / sizeFactor
        console.log("UPDATING", sc1, sc2, sc3, sc4)
        context.save()
    ...
    static drawEAHNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawExpandArmH(context, scale)
    } 
}
