...
class ASBLDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new ASBLDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawASBLDNode(context, this.i, this.state.scale)
    }
...
class AltSideBarLineDown {

    curr : ASBLDNode = new ASBLDNode(0)
    dir : number = 1 
