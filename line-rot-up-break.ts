...
class DrawingUtil {
    ...
    static drawLineRotUpBreak(context: CanvasRenderingContext2D, scale: number) {
        const size: number = Math.min(w, h) / sizeFactor
        const dsc: (a: number) => number = (i: number): number => ScaleUtil.divideScale(scale, i, parts)
        DrawingUtil.drawXY(context, w / 2, h / 2, () => {
            context.rotate(rot * dsc(1))
            for (let j = 0; j < lines; j++) {
                DrawingUtil.drawBrokenLine(context, j, size, dsc(0), dsc(2 + (lines - 1 - j)))
            }
        })
    }

    static drawLRUBNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineRotUpBreak(context, scale)
    }
}
...
class Animator {
    ...
    stop() {
        if (this.animated) {
            this.animated = false
            console.log("STOP ANIMATED", this.animated)
...
class LRUBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LRUBNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLRUBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LRUBNode {
        var curr: LRUBNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineRotUpBreak {

    curr: LRUBNode = new LRUBNode(0)
    dir: number = 1
