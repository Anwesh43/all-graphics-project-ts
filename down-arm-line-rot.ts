...
class DrawingUtil {
    ...
    static drawDownArmLineRot(context : CanvasRenderingContext2D, scale : number) {
...
class DALRNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new DALRNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawDALRNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : DALRNode {
        var curr : DALRNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class DownArmLineRot {

    curr : DALRNode = new DALRNode(0)
    dir : number = 1
