...
class DrawingUtil {
    ...
    static drawLeftPartialLineBent(context: CanvasRenderingContext2D, scale: number) {
        ...
        DrawingUtil.drawXY(context, w / 2 - (w / 2) * dsc(4), h / 2, () => {
            ...
            DrawingUtil.drawXY(context, 0, 0, () => {
                context.rotate(deg * (1 - dsc(3)))
                DrawingUtil.drawLine(context, 0, 0, -size * dsc(1), 0)
            })
        })
    }

    static drawLPLBNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLeftPartialLineBent(context, scale)
    }
}
...
class LeftPartialLineBent {

    curr: LPLBNode = new LPLBNode(0)
    dir: number = 1
