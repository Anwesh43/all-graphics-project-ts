...
class PASRDNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new PASRDNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawPASRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : PASRDNode {
        var curr : PASRDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class PlusAboveSquareRotDown {

    curr : PASRDNode = new PASRDNode(0)
    dir : number  = 1
