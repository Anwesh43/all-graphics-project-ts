...
class Animator {
 ...
 class BRAPNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BRAPNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBRAPNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : BRAPNode {
        var curr : BRAPNode | null  = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr != null) {
 ...
 class BlockRightAnglePoint {

    curr : BRAPNode = new BRAPNode(0)
    dir : number = 1
