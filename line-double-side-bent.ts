...
class DrawingUtil {
    ...
    static drawLDSBNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineDoubleSideBent(context, scale)
    }
}
...
class LDSBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LDSBNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLDSBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LDSBNode {
        var curr: LDSBNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
...
class LineDoubleSideBent {

    curr: LDSBNode = new LDSBNode(0)
    dir: number = 1
