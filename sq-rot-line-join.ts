...
class SRLJNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SRLJNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSRLJNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : () => void) : SRLJNode {
        var curr : SRLJNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
