...
class CBMCNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new CBMCNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawCBMCNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : CBMCNode {
        var curr : CBMCNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class ColorBarMidCircle {

    curr : CBMCNode = new CBMCNode(0)
    dir : number = 1
