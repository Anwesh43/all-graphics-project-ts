...
class DrawingUtil {
    ...
    static drawArc(context: CanvasRenderingContext2D, r: number, sweep: number) {
        const endDeg: number = Math.floor(sweep) + 180
        context.beginPath()
        for (let j = 180; j <= endDeg; j++) {
    ...
    static drawLineArcJoinPerp(context: CanvasRenderingContext2D, scale: number) {
    ...
    static drawLAJPNode(context: CanvasRenderingContext2D, i: number, scale: number) {
        context.lineCap = 'round'
        context.lineWidth = Math.min(w, h) / strokeFactor
        context.strokeStyle = colors[i]
        DrawingUtil.drawLineArcJoinPerp(context, scale)
    }
}
...
class LAJPNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LAJPNode(this.i + 1)
            this.next.prev = this
        }
    }

    draw(context: CanvasRenderingContext2D) {
        DrawingUtil.drawLAJPNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir: number, cb: () => void): LAJPNode {
        var curr: LAJPNode = this.prev
        if (dir === 1) {
            curr = this.next
        }
        if (curr) {
